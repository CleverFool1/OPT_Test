...
def plot_3d(ax):
    ...
    ax.plot_surface(X, Y, F1(X, Y), rstride=1, cstride=1, cmap=cm.coolwarm)
    ax.set_zlim(-10, 10)
    ax.set_xlabel('x')
    ax.set_ylabel('y')
    ax.set_zlabel('z')
...
def print_info(pop):
    ...
    # x = X[0]
    # y = X[1]
    print("(x, y):", (X[0][max_fitness_index], X[1][max_fitness_index]))
...
if __name__ == "__main__":
    ...
    for _ in range(N_GENERATIONS):  # 迭代N代
        ...
        if 'sca' in locals():
            sca.remove()
        sca = ax.scatter(X[0], X[1], F(X), c='black', marker='o')
        plt.show()
        plt.pause(0.1)
